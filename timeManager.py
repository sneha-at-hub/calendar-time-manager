...
def main():
    ...
    """
    if len(argv) < 2:
        print("Usage: python timeManager.py <command> [options]")
        return
    ...
    if command == 'add':
        if len(argv) < 4:
            print("Usage: python timeManager.py add <duration> <description>")
            return
        duration = argv[2]
        description = argv[3]
        ...
        if len(argv) < 3:
            print("Usage: python timeManager.py get <number_of_days>")
            return
        number_of_days = argv[2]
        getHours(number_of_days)
    else:
        print(f"Unknown command: {command}")
        print("Usage: python timeManager.py <add|commit|get> [options]")
...
def commitHours(creds):
    try:
        ...
        events_result = service.events().list(
            calendarId=YOUR_CALENDAR_ID, timeMin=timeStart, timeMax=timeEnd, singleEvents=True, orderBy='startTime', timeZone=YOUR_TIMEZONE).execute()
